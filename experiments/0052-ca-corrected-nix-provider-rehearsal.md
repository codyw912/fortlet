...
Status: completed — rejected with unresolved base-packaging assumption
Design: FIP-0012
...
The network-approved first prepare downloaded the common image, created the
credential-free base provisioning capsule, and reached the new
`certificate-bundle` diagnostic step. It exited 2 with the exact actionable
error that `/out/etc/ca-certificates.conf` did not exist. Tact and schema-2 Nix
preparation never began.

The correction's synthetic fixture had supplied that configuration file and
therefore retained the production assumption it was meant to falsify. Debian's
package-installation lifecycle creates both the selection configuration and
the certificate bundle; raw `dpkg-deb -x` extraction creates neither. The
command-level diagnostic correction worked, but the CA construction did not.
...
1. Outcome: rejected before harness or Nix-provider preparation.
2. Root cause: the attempted fix assumed raw package extraction includes
   `/etc/ca-certificates.conf`; it is generated by Debian's package lifecycle,
   so the bundle builder has no selection input.
...
5. Next action: stop. Do not apply another path-level correction. Reconsider
   the shared raw-package-extraction assumption and predeclare an end-to-end
   base-layer rehearsal only under new operator direction.
