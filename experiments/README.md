...
- `0051-firewall-cleared-nix-provider-rehearsal.md` — rejected with a Fortlet defect; network approval cleared Docker Hub DNS, then common-base extraction required a generated CA bundle that it never generated; cleanup completed before Tact or Nix.
...
- `0057-sealed-schema-one-lifecycle.md` — rejected before schema-1 publication; the first cold attempt and one operator-directed firewall retry both lost preparation-guest DNS for Debian despite successful intervening host HTTPS clearance, so no warm or lifecycle sample ran and exact cleanup completed.
