...
impl Fixture {
    ...
    fn seed_base(&self) {
        seed_marker(&self.tools().join("_base/bookworm-1"), ".fortlet-base.json");
    }
...
fn cache_hits_are_silent_idempotent_and_credential_free_for_both_harnesses() {
    for harness in ["codex", "tact"] {
        ...
        let before = fs::read(fixture.tools().join("_base/bookworm-1/.fortlet-base.json")).unwrap();
        ...
        assert_eq!(
            fs::read(fixture.tools().join("_base/bookworm-1/.fortlet-base.json")).unwrap(),
            before
        );
        fixture.assert_no_runtime_state();
    }
...
fn incomplete_harness_layer_preserves_the_verified_base() {
    let fixture = Fixture::new();
    fixture.seed_base();
    let base_marker = fixture.tools().join("_base/bookworm-1/.fortlet-base.json");
